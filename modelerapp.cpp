                              ...
                              const ModelerControl controls[], unsigned numControls)
{
	...
	// TMP
	m_ui->addControl("LookAt X",  -500,  500, 0.5f,   0.0f);
	m_ui->addControl("LookAt Y",  -500,  500, 0.5f,   0.0f);
	m_ui->addControl("LookAt Z",  -500,  500, 0.5f,   0.0f);
...
int ModelerApplication::Run()
{
    ...
    // Just tell FLTK to go for it.
   	Fl::visual( FL_RGB | FL_DOUBLE | FL_MULTISAMPLE );
	m_ui->show();
