...
void RobotArm::draw()
{
	...
	// set the particle pos
	Mat4f modelView = glGetMatrix(GL_MODELVIEW_MATRIX);
	Vec4f pos = matCamInverse * modelView * Vec4f(0, 0, 0, 1.f);

	glPopMatrix();

	if (particles != nullptr)
	{
		particles->setEmitRate(pc);
		particles->setSource(Vec3d(pos[0], pos[1], pos[2]));
		particles->forcedDraw(t);
	}
...
int main(int argc, char* argv[])
{
    ...
    controls[PARTICLE_COUNT] = ModelerControl("particle emission count (pc)", 0.0, 5.0, 0.1, 5.0 );
    
	// You should create a ParticleSystem object ps here and then
	// call ModelerApplication::Instance()->SetParticleSystem(ps)
	// to hook it up to the animator interface.
	particles = new ParticleSystem();
	ModelerApplication::Instance()->SetParticleSystem(particles);
